...
- 📚 **Module-Based Core**
  The engine now ships as C++23 modules so ECS partitions and runtime systems can be imported directly. Pull the aggregate via `import aengine;` or target partitions such as `import aengine.engine_components;`, `import aengine.platform;`, `import aengine.renderers;`, `import aengine.menu;`, `import aengine.input;`, or the updater slices (`import aengine.updater;`, `.updater.config`, `.updater.tools`). Context/back-end wiring sits on module surfaces as well: `import aengine.context;`, `import aengine.context.window;`, `import aengine.context.render;`, `import aengine.context.opengl;`, `import aengine.context.sdl;`, `import aengine.context.raylib;`, `import aengine.context.software;`, plus backend entries under `aengine.opengl.*` and `aengine.sdl.*` for renderer-specific imports. Recent header-to-module conversions also expose direct imports for atlas helpers (`import aatlas.manager;`, `import aatlas.texture;`), command-line parsing (`import acommandline;`), entity/component plumbing (`import aentity.*` partitions), and umbrella aliases (`import almondengine;`, `import almondshell;`) so mixed include/import consumers stay compatible during migration.
- 🧠 **Functional Flow**
...
- ✅ **Header-to-module conversions** – Atlas helpers (`aatlasmanager.hpp`, `aatlastexture.hpp`) and CLI plumbing (`acommandline.hpp`) are now first-class module partitions (`import aatlas.manager;`, `import aatlas.texture;`, `import acommandline;`) alongside new ECS mirrors (`import aentity.*`) and umbrella aliases (`import almondengine;`, `import almondshell;`) to simplify mixed import/include builds.

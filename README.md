...
- 📚 **Module-Based Core**
  The engine now ships as C++23 modules so ECS partitions and runtime systems can be imported directly. Pull the aggregate via `import aengine;` or target partitions such as `import aengine.engine_components;`, `import aengine.platform;`, `import aengine.systems;`, `import aengine.gui;`, `import aengine.gui.menu;`, `import aengine.input;`, `import aengine.cli;`, and the updater slices (`import aengine.updater;`, `.updater.config`, `.updater.system`, `.updater.tools`). Context/back-end wiring sits on module surfaces as well: `import aengine.context.commandqueue;`, `import aengine.context.control;`, `import aengine.context.multiplexer;`, `import aengine.context.type;`, `import aengine.context.window;`, and `import aengine.context.sdl;`, plus backend entries under `acontext.opengl.*`, `acontext.sdl.*`, `acontext.raylib.*`, and `acontext.softrenderer.*` (along with `acontext.noop.context`) for renderer-specific imports. Recent header-to-module conversions also expose direct imports for atlas helpers (`import aatlas.manager;`, `import aatlas.texture;`), command-line parsing (`import aengine.core.commandline;`), entity/component plumbing (`import aentity;`, `import aentity.component.manager;`, `import aecs.*` partitions), and umbrella aliases (`import almondshell;`) so mixed include/import consumers stay compatible during migration.
- 🧠 **Functional Flow**
...
- ✅ **Updater metadata consistency** – Updater/configuration guidance now calls out that version strings are sourced from the runtime helpers, so manifests and launcher metadata should mirror the v0.81.23 tag.
...
- ✅ **Context/back-end module coverage** – The multiplexer, window wiring, and backend bindings (OpenGL, SDL3, Raylib, and the software renderer) continue to ship as `.ixx` partitions so module consumers can `import aengine.context.commandqueue`, `import aengine.context.multiplexer`, `import aengine.context.type`, `import aengine.context.window`, `import aengine.context.sdl`, or target backend-specific `acontext.*` modules without relying on legacy headers. Mirrored headers (e.g., `aengine.hpp`, `aplatform.hpp`, `acontext*.hpp`) remain wired into the matching partitions for compatibility.
- ✅ **C++23 + module-first baseline** – Documented the full shift to a C++23 toolchain and module-first build flow so packagers and tooling can align module scanning, BMI generation, and compiler flags with the current engine expectations.
...
- [`AlmondShell/docs/engine_analysis.md`](AlmondShell/docs/engine_analysis.md) and [`AlmondShell/docs/context_audit.md`](AlmondShell/docs/context_audit.md) catalogue current focus areas and cleanup candidates for future contributors.

...
- ✅ **HiDPI-aware multi-context rendering** – Raylib now seeds its viewport fit immediately during resize events, eliminating the start-up clipping and scaling drift that previously nudged menu buttons off-screen, while SDL3 continues to remap input so docked panes stay pixel-accurate after DPI or window changes.

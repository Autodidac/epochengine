...
- **v0.70.4** – Documented that the OpenGL renderer now binds the backend-managed GL state so quad and debug-outline helpers reuse the pipeline initialised by `opengl_initialize()` on both Windows and Linux.

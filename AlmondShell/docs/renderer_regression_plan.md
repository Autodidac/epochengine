...
| SDL | `--renderer=sdl --scene=menu_rainbow` | *Renderer output sizing:* `SDL_GetCurrentRenderOutputSize` should track Win32 client changes.<br>*GUI parity:* Menu overlay layout and input scaling match Raylib/OpenGL (button bounds centered, hover/click hitboxes align with sprites).<br>*Event pump:* Escape key exits, resize storm drained without blocking queue.<br>*Atlas upload:* SDL uploader flushes textures. | Background hue cycling without hitching, menu buttons stay centered with accurate hover/click feedback, resize events leave canvas centred, command queue drains to empty each frame. |
...
| SDL | `./Bin/GCC-Debug/cmakeapp1/cmakeapp1 --renderer=sdl --scene=menu_rainbow --trace-menu-button0` | `[MenuOverlay] button0 GUI bounds: x=272.00 y=328.00 w=320.00 h=120.00` (matches Raylib/OpenGL layout scaling) |

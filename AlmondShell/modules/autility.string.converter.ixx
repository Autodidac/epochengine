...
export namespace almondnamespace::text {
    ...
    export inline std::string narrow_utf8(const std::wstring& wide)
    {
        ...
        if (size <= 0) return {};
        std::string out(static_cast<size_t>(size), L'\\');
        // Convert
    ...
    export inline std::wstring widen_utf16(const std::string& utf8)
    {
        ...
        int size = ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
            ...
            nullptr, 0);
        if (size <= 0) return {};
        std::wstring out(static_cast<size_t>(size), L'\\');

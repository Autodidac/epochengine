...
export namespace almondnamespace::mem
{
    ...
    template<typename T>
    void destroy_frame(T* obj) {
        if (obj) obj->~T();
    }

    template<typename T>
    [[nodiscard]] std::pmr::polymorphic_allocator<T> frame_allocator() noexcept {
        return std::pmr::polymorphic_allocator<T>{ &frame_arena() };
    }

    template<typename T>
    using frame_vector = std::pmr::vector<T>;
}

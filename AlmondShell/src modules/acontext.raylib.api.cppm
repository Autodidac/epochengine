...
// DO NOT macro-rename raylib exports. That breaks linking.
// If Win32 headers are visible elsewhere, avoid collisions by never exposing
// raw raylib names outside this module.
...
{
    // --- Export the raylib POD types so other modules don't include raylib.h ---
    export using ::Color;
    ...
    // --- Mouse button constants needed by input ---
    ...
    // --- Pixel format constant used by textures ---
    ...
    // In docked / external-window mode you generally do NOT want raylib
    // owning the window. So we expose both variants:
...
#else
        // In no-window mode, assume render path is ready if you got here.
        return true;
#endif
    }
    ...
    export inline void set_window_title(const char* title) { ::SetWindowTitle(title); }

    // --- Input queries (used by input module) ---
    export inline bool is_key_down(int raylib_key) { return ::IsKeyDown(raylib_key); }
    export inline bool is_mouse_button_down(int raylib_btn) { return ::IsMouseButtonDown(raylib_btn); }
    ...
    // --- Textures (used by textures + renderer) ---
    export inline ::Texture2D load_texture_from_image(const ::Image& img)
    {
        return ::LoadTextureFromImage(img);
    }

    export inline void unload_texture(const ::Texture2D& tex)
    {
        ::UnloadTexture(tex);
    }

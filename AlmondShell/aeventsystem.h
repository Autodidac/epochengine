...
#undef KeyPress // naming collision on windows platform
    ...
    struct Event {
        almondnamespace::EventType type{};
    ...
    public:
        void PollEvents();
        void RegisterCallback(const std::function<void(const almondnamespace::Event&)>& callback);
        /*
            // Add a movement event as a unique pointer
            void addMovementEvent(std::unique_ptr<almondnamespace::MovementEvent> event) {
            ...
            std::vector<std::unique_ptr<almondnamespace::MovementEvent>> getMovementEvents() {
                std::lock_guard<std::mutex> lock(movementMutex);
                std::vector<std::unique_ptr<almondnamespace::MovementEvent>> eventsCopy;
    ...
    std::string EventTypeToString(almondnamespace::EventType type);
    almondnamespace::EventType StringToEventType(const std::string& str);
}